...
class NonadaptiveGRU(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, straight_through=False, scale_sigmoid=5.0):
        ...
        self.sel_logits = nn.Parameter(torch.empty(self.input_size).normal_(mean=0, std=1e-1), requires_grad=False)
    ...
    def forward(self, input):
        ...
        for i in range(seq_length):
            if i == 0:
                ...
                #weights1 = weights1.squeeze(-1) # batch x feature
                #weights2 = weights2.squeeze(-1) # batch x feature
                ...
                weights1 = self.categorical_sample(self.sel_logits.unsqueeze(-1) * self.scale_sigmoid, u, anti=False)
                weights2 = self.categorical_sample(self.sel_logits.unsqueeze(-1) * self.scale_sigmoid, u, anti=True)
                ...
                if self.straight_through:
                    st1 = (weights1 - F.sigmoid(self.sel_logits*self.scale_sigmoid).unsqueeze(0)).detach() + F.sigmoid(self.sel_logits*self.scale_sigmoid).unsqueeze(0)
                    st2 = (weights2 - F.sigmoid(self.sel_logits*self.scale_sigmoid).unsqueeze(0)).detach() + F.sigmoid(self.sel_logits*self.scale_sigmoid).unsqueeze(0)
